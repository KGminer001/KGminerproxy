...
wdog='runningFXMPV3'
installfolder='/etc/frp-fxminerproxyv3/runningFXMPV3'
installdir='/etc/frp-fxminerproxyv3/'
...
install() {
    ...
    if [ ! -f "$installfolder" ]; then
        ...
        if [ -f "$version.tar.gz" ]; then
            ...
            mkdir frp-$sofname && chmod 777 frp-$sofname
            #判断文件夹是否创建成功
            if [ ! -d "frp-$sofname" ]; then
                echo && echo -n -e "${yellow}安装失败,请重新操作: ${plain}" && read temp
                rm -rf frp-$sofname
                rm -rf fxminerproxyv3linux && rm $version.tar.gz
                return
            fi
            mv fxminerproxyv3linux/$sofname frp-$sofname
            mv fxminerproxyv3linux/running.sh frp-$sofname/$wdog
            cd frp-$sofname && chmod +x $wdog && chmod +x $sofname && cd ../
            cp -r frp-$sofname /etc/
            rm -rf frp-$sofname
            rm -rf fxminerproxyv3linux && rm $version.tar.gz
            echo -e "删除安装包"
            if [ ! -f "$installfolder" ]; then
...
update_app() {
    ...
    rm /etc/frp-$sofname/*.cache
    rm /etc/frp-$sofname/$bin_name.bin
    kill_wdog
    killProcess
    tar -zxvf $version.tar.gz
    mkdir frp-$sofname && chmod 777 frp-$sofname
    ...
    if [ ! -d "frp-$sofname" ]; then
        ...
        rm -rf fxminerproxyv3linux && rm $version.tar.gz
        if [ ! -f "$installfolder" ]; then
            echo && echo -n -e "${yellow}更新失败,请重新打开脚本操作"
            return
        else
...
start() {
    if [ ! -f "$installfolder" ]; then
        ...
        if [ $? -eq 1 ]; then
            ...
            echo -e ${clearscr}
            setsid ./$wdog -franchisee=$bin_path/$bin_name &
            sleep 3
        fi
    fi
    before_show_menu
...
checkConfigFile(){
    cat /etc/frp-fxminerproxyv3/localconfig.json
}
delErrFile(){
    echo "" >/etc/frp-fxminerproxyv3/error.log
...
show_menu() {
    ...
    echo -e "
     ${yellow}${a_name}
     ${green}脚本管理界面安装完成(建议使用debian8.*以上版本内存控制更好)
